...
class UserController extends Controller
{
    ...
    public function openAICustomDelete($id = null)
    {
        ...
        return back()->with(['message' => __('Excluido com Sucesso'), 'type' => 'success']);
    }
    ...
    public function openAIGeneratorWorkbook($slug)
    {
        ...
        if ($slug == 'ai_vision' || $slug == 'ai_pdf' || $slug == 'ai_chat_image') {
            ...
            if ($isPaid == false && $category->plan == 'premium' && auth()->user()->type !== 'admin') {
                return redirect()->back()->with(['message' => __('Precisa de um Plano'), 'type' => 'error']);
            }
    ...
    public function userSettingsUpdate(Request $request): RedirectResponse
    {
        ...
        if (! $user = Auth::user()) {
            return redirect()->back()->with(['message' => __('Sem Autorização'), 'type' => 'error']);
        }
        ...
        return redirect()->back()->with(['message' => __('Atualizado com Sucesso'), 'type' => 'success']);
    }
    ...
    public function updateFolder(Request $request, $folder)
    {
        ...
        return response()->json(['message' => __('Nome da Pasta Atualizada')]);
    }
    ...
    public function updateFile(Request $request, $slug)
    {
        ...
        return response()->json(['message' => __('Nome do Arquivo Atualizado')]);
    }
    ...
    public function deleteFolder(Request $request, $folder)
    {
        ...
        return response()->json(['message' => __('Pasta Deletada')]);
    }
    ...
    public function newFolder(Request $request)
    {
        ...
        return back()->with(['message' => __('Adicionado com Sucesso'), 'type' => 'success']);
    }
    ...
    public function moveToFolder(Request $request)
    {
        ...
        return back()->with(['message' => __('Movida com Sucesso'), 'type' => 'success']);
    }
    ...
    public function documentsDelete($slug)
    {
        ...
        return redirect()->route('dashboard.user.openai.documents.all')->with(['message' => __('Documento excluido'), 'type' => 'success']);
    }
    ...
    public function deleteAccountRequest(Request $request)
    {
        ...
        if (Hash::check($request->password, $user->password)) {
            ...
            if ($oldRequest) {
                return response()->json(['message' => __('Você já solicitou a exclusão da conta')], 409);
            }
            $deletionRequest = new AccountDeletionReqs;
            $deletionRequest->user_id = $user->id;
            $deletionRequest->save();

            return response()->json(['message' => __('A solicitação da exclusão da sua conta foi feita com sucesso')], 200);
        } else {
            return response()->json(['message' => __('Senha está incorreta')], 401);
        }
    }
}
