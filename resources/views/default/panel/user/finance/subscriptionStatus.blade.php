...
@else
    ...
    <p class="mb-3 font-medium leading-relaxed text-heading-foreground/60">
        @if (Auth::user()->activePlan() != null)
            ...
            {{ checkIfTrial() == true ? __('Você está no período de avaliação.') : '' }}
        @else
            {{ __('Você não possui uma assinatura no momento. Por favor, selecione um plano de assinatura.') }}
        @endif
    ...
    <div class="mt-6 flex flex-wrap items-center justify-center gap-4">
        ...
        >
            ...
            {{ __('Selecione um Plano') }}
        </x-button>

        @if (getSubscriptionStatus())
            <x-button
                variant="danger"
                onclick="return confirm('Tem certeza de que deseja cancelar seu plano? Você perderá o uso restante.');"
