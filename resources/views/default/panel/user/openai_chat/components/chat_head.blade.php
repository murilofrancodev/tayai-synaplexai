<div
    ...
    <div class="flex grow items-center justify-end gap-4">
        <div class="flex gap-2">
            @includeIf('panel.user.openai_chat.includes.share-button-include')
            ...
            @else
                <x-forms.input
                        class="max-md:hidden"
                        id="realtime"
                        container-class="{{ $category->slug == 'ai_pdf' || $category->slug == 'ai_vision' ? 'hidden' : '' }} max-md:size-8 max-md:inline-flex max-md:items-center max-md:justify-center max-md:overflow-hidden max-md:shadow-md max-md:rounded-full max-md:shrink-0 max-md:[&_.lqd-input-label-txt]:hidden"
            ...
            @else
                ...
                <div class="lqd-chat-mobile-sidebar-trigger self-center">
                    ...
                    >
                        <x-tabler-dots
                                class="size-5 col-start-1 row-start-1 transition-all group-[&.active]:rotate-45 group-[&.active]:scale-75 group-[&.active]:opacity-0"/>
                        <x-tabler-x
                                class="size-4 col-start-1 row-start-1 -rotate-45 opacity-0 transition-all group-[&.active]:rotate-0 group-[&.active]:!opacity-100"/>
                    </button>
                </div>
            @endif
        </div>
